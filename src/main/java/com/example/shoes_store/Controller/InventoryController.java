...
import com.example.shoes_store.dto.ExportReceiptDTO;
...
//        User loggedInUser = (User) session.getAttribute("loggedInUser");
    ...
    public Map<String, Object> getHistory(@RequestParam(required = false) String type) {
        ...
        if (type == null || "all".equals(type)) {
            result.put("imports", inventoryService.getAllImportReceiptsList());  // Dùng DTO mới
            result.put("exports", inventoryService.getAllExportReceiptsList());  // Dùng DTO mới
        } else if ("import".equals(type)) {
            result.put("imports", inventoryService.getAllImportReceiptsList());
            result.put("exports", List.of());
        } else if ("export".equals(type)) {
            result.put("imports", List.of());
            result.put("exports", inventoryService.getAllExportReceiptsList());
        }
    ...
    @ResponseBody
    public ExportReceiptDTO getExportDetail(@PathVariable Long id) {
